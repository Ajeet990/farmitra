<div>
    ...
    @endif
    <h1>Vendor Premium History List</h1>
    <div class="tw-flex tw-font-bold tw-justify-between tw-items-center tw-p-2">
      ...
      <div class="tw-flex tw-justify-content-end tw-items-center">
         ...
         <a href="{{ route("add.vendor.premium.history") }}" class="tw-bg-blue-700 tw-px-2 tw-py-1 tw-text-white  tw-rounded-md tw-whitespace-nowrape">Add +</a>
      </div>
   </div>
        ...
        <table class="tw-min-w-full tw-divide-y tw-divide-gray-200">
          <thead class="tw-bg-gray-50">
            <tr>
              ...
              <th class="tw-px-6 tw-py-3 tw-text-left tw-text-xs tw-font-medium tw-text-gray-500 tw-uppercase tw-tracking-wider">Premium Package</th>
              <th class="tw-px-6 tw-py-3 tw-text-left tw-text-xs tw-font-medium tw-text-gray-500 tw-uppercase tw-tracking-wider">expired</th>
              <th class="tw-px-6 tw-py-3 tw-text-left tw-text-xs tw-font-medium tw-text-gray-500 tw-uppercase tw-tracking-wider">expired_on</th>
          ...
          <tbody class="tw-bg-white tw-divide-y tw-divide-gray-200">
            @foreach ($vendorPremiumHistory as $item)
            <tr>
                ...
                <td class="tw-px-6 tw-py-4 tw-whitespace-nowrap">{{ $item->premium_package->name }}</td>
                <td class="tw-px-6 tw-py-4 tw-whitespace-nowrap">{{ $item->expired }}</td>
                <td class="tw-px-6 tw-py-4 tw-whitespace-nowrap">{{ $item->expired_on }}</td>
                <td class="tw-px-6 tw-py-4 tw-whitespace-nowrap">{{ $item->remark }}</td>
                <td>
                    <a href="{{ route('edit.vendor.premium.history',$item->id )}}" class="tw-bg-green-100 tw-text-blue-800 tw-text-xs tw-font-semibold tw-px-2 tw-rounded-full tw-uppercase ">edit</a>
