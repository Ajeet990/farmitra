<div>
    ...
    <div class="tw-flex tw-font-bold tw-justify-between tw-items-center">
        <h1>Edit Legal Documents</h1>
        <a href="{{ route('list.legal.documents') }}" class="tw-bg-blue-700 tw-px-2 tw-py-1 tw-text-white  tw-rounded-md"><-- Back</a>
    </div>
    <div class="tw-max-w-md tw-mx-auto tw-bg-white tw-rounded-lg tw-shadow-md tw-p-6">
        ...
        <div class="tw-mb-4">
            <label for="sample" class="tw-block tw-text-sm tw-font-medium tw-text-gray-700">Sample</label>
            <input type="text" wire:model="sample"  placeholder="Sample" class="tw-mt-1 tw-block tw-w-full tw-border-gray-300 tw-rounded-md tw-shadow-sm tw-focus:ring-indigo-500 tw-focus:border-indigo-500 tw-sm:text-sm">
            @error('description')  
            <span class="tw-text-red-500 tw-text-xs tw-mt-3 tw-block ">{{ $message }}</span>
          @enderror
        </div>
